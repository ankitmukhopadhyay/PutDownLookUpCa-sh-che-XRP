...
def setup_demo():
    ...
    for user in DEMO_USERS:
        ...
        # Top up platform wallet before each user (each costs ~42 XRP)
        platform_xrp = get_xrp_balance(client, platform_wallet.address)
        log(f"Platform balance: {platform_xrp} XRP")
        if platform_xrp < 50:
            log("Low balance — requesting faucet top-up...")
    ...
    if "Alice" not in existing or "Bob" not in existing:
        ...
        # Top up platform wallet for simulation payments
        platform_xrp = get_xrp_balance(client, platform_wallet.address)
        if platform_xrp < 20:
            log("Topping up platform for simulation...")
            faucet_topup(platform_wallet.address)
